...
/**
 * Check that property is renderer as a URL with expected state
 * @param {String} bunsenId - bunsen ID for property rendered as URL
 * @param {Object} state - expected state of URL renderer
 */
export function expectWithState (bunsenId, state) {
  ...
  expect(
    ...
    .to.have.class('frost-bunsen-input-url')
  ...
  expect(
    ...
    'URL renderer has expected placeholder text'
  )
    .to.equal(state.placeholder)

  expect(
    $input.val(),
    'URL renderer input has expected value'
  )
